...
if __name__ == "__main__":
    ...
    experiment_keys = client.get_experiments('gBm_delta_donsker_qc_fourier',
                                             'trans_backend=ibmqx2', 'version=v001', 'K=110')
    ...
    strike_price, [experiment, mitigation] = list(prepared_experiments.items())[0]
    ...
    initial_value = experiment.parameters.get('initial_value')
    probabilities = experiment.parameters.get('probabilities')
    realizations = experiment.parameters.get('realizations')
    evaluations = experiment.arguments
    theory_char_func_output = np.asarray(
        benchmark.characteristic_function_rw_ind(initial_value, probabilities, realizations, evaluations)
    )

    fig = plot_characteristic_function(
        simulation=sim_char_func_output,
        experiment=char_func_output,
        theory=theory_char_func_output,
        title=f"Experiment on {experiment.execution_backend_name}\n"
    ...
    df = pd.DataFrame(
        data=zip(np.asarray(evaluations),
                 np.real(theory_char_func_output), np.imag(theory_char_func_output),
                 np.real(sim_char_func_output), np.imag(sim_char_func_output),
                 np.real(char_func_output), np.imag(char_func_output)
                 ),
        columns=['evaluations', 'theory (real)', 'theory (imag)', 'sim (real)', 'sim (imag)',
                 'exp (real)', 'exp (imag)']
    )

    df.to_csv(f'../../data/gBm-delta-donsker-{n}-fourier-qc-details-{experiment.execution_backend_name}-P={P}-L={L}-K={strike_price}.csv')
